...
sourceInfo = dir(options.sourceFile);
jarInfo = dir(options.jarFile);
signature = sprintf([ ...
    'compiler=%s\nmajor=%d\nflags=%s\n', ...
    'source=%s|%d|%.15g\njar=%s|%d|%.15g\n'], ...
    strtrim(versionOutput), majorVersion, compilerFlags, ...
    char(options.sourceFile), sourceInfo.bytes, sourceInfo.datenum, ...
    char(options.jarFile), jarInfo.bytes, jarInfo.datenum);
