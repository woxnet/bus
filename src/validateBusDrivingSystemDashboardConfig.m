...
defaults=struct('refreshHz',5,'maximumRefreshHz',10, ...
...
numeric={'refreshHz','maximumRefreshHz','signalHistorySeconds','maximumEventRows', ...
    'maximumLogRows','maximumStageHistory','maximumRenderMilliseconds'};
for k=1:numel(numeric)
    validateattributes(config.(numeric{k}),{'numeric'},{'scalar','real','finite','positive'});
end
integerNames={'maximumEventRows','maximumLogRows','maximumStageHistory'};
