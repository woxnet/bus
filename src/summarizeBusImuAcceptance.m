...
summary.runtimeTailComplete=reportFieldTrue(runtimeReport, ...
    'samplesReadMatchesReceived') && reportFieldFalse(runtimeReport, ...
    'stopDrainTimedOut');
summary.runtimeBufferEmpty=reportFieldEquals(runtimeReport, ...
    'finalBufferedSamples',0);
...
summary.success=summary.calibrationSuccess && summary.runtimeSuccess && ...
    ...
    summary.runtimeTailComplete && summary.runtimeBufferEmpty;
end
...
function result=reportFieldTrue(report,name)
result=isstruct(report) && isscalar(report) && isfield(report,name) && ...
    scalarTrue(report.(name));
end

function result=reportFieldFalse(report,name)
result=isstruct(report) && isscalar(report) && isfield(report,name) && ...
    (islogical(report.(name))||isnumeric(report.(name))) && ...
    isscalar(report.(name)) && isfinite(double(report.(name))) && ...
    ~logical(report.(name));
end

function result=reportFieldEquals(report,name,expected)
result=isstruct(report) && isscalar(report) && isfield(report,name) && ...
    isnumeric(report.(name)) && isscalar(report.(name)) && ...
    isfinite(double(report.(name))) && double(report.(name))==expected;
end
