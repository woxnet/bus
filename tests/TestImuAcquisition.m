...
%TESTIMUACQUISITION Hardware-independent acquisition and persistence tests.
    ...
    methods (Test)
        ...
        function recorderWritesBoundedChunksWithoutDuplicates(testCase)
            ...
            testCase.verifyGreaterThan(session.durationSeconds, 0);
        ...
        function completeAcceptanceSummaryPasses(testCase)
            startupStatus = struct('available', true, ...
                'restartRequired', false, 'loadedSourcesMatch', true, ...
                'javaAddPathCalled', false, 'pathsAdded', strings(0, 1), ...
                'bridgeClassLoaded', true, 'warnings', strings(0, 1));
            unitResults = struct('Passed', {true, true}, ...
                'Failed', {false, false}, 'Incomplete', {false, false}, ...
                'Duration', {1.25, 0.75});
            preflight = struct('success', true, 'warnings', strings(0, 1));
            hardware = struct('success', true, 'durationSeconds', 60.1, ...
                'missing', 0, 'overflowDropped', 0, 'staleSessionDropped', 0, ...
                'meanFrequencyHz', 50, 'sampleAgeMaximumMs', 2, ...
                'gravityMagnitudeMean', 9.81, 'quaternionNormMean', 1, ...
                'warnings', strings(0, 1));
            recorded = struct('status', "complete", 'durationSeconds', 120.1, ...
                'samplesWritten', 6000, 'duplicateSamples', 0, ...
                'missingSamples', 0, 'overflowDropped', 0, ...
                'staleSessionDropped', 0, ...
                'directory', string(testCase.TemporaryDirectory), ...
                'warnings', strings(0, 1));
            summary = summarizeImuAcquisitionAcceptance(startupStatus, ...
                unitResults, preflight, hardware, recorded, ...
                testCase.TemporaryDirectory);
            testCase.verifyTrue(summary.success);
            testCase.verifyEqual(summary.unitTests.Passed, 2);
            testCase.verifyEqual(summary.unitTests.Failed, 0);
            testCase.verifyEqual(summary.unitTests.Incomplete, 0);
            testCase.verifyEqual(summary.unitTests.Duration, 2);
            testCase.verifyTrue(isfile(summary.matFile));
            testCase.verifyTrue(isfile(summary.jsonFile));
        end

        function incompleteHardwareMakesSummaryFail(testCase)
            startupStatus = struct('available', true, ...
                'restartRequired', false, 'loadedSourcesMatch', true, ...
                'javaAddPathCalled', false, 'pathsAdded', strings(0, 1), ...
                'bridgeClassLoaded', true);
            unitResults = struct('Passed', true, 'Failed', false, ...
                'Incomplete', false, 'Duration', 1);
            preflight = struct('success', true);
            hardware = struct('success', true, 'durationSeconds', 60, ...
                'missing', 1, 'overflowDropped', 0, 'staleSessionDropped', 0, ...
                'meanFrequencyHz', 50, 'sampleAgeMaximumMs', 2, ...
                'gravityMagnitudeMean', 9.81, 'quaternionNormMean', 1);
            recorded = struct('status', "complete", 'durationSeconds', 120, ...
                'samplesWritten', 6000, 'duplicateSamples', 0, ...
                'missingSamples', 0, 'overflowDropped', 0, ...
                'staleSessionDropped', 0, ...
                'directory', string(testCase.TemporaryDirectory), ...
                'warnings', 'single readable warning');
            summary = summarizeImuAcquisitionAcceptance(startupStatus, ...
                unitResults, preflight, hardware, recorded, ...
                testCase.TemporaryDirectory);
            testCase.verifyFalse(summary.success);
            testCase.verifyFalse(summary.hardwareAcceptance.success);
            testCase.verifyTrue(any(contains(summary.errors, ...
                "hardwareAcceptance")));
            testCase.verifyEqual(summary.warnings, "single readable warning");
        end
    end
